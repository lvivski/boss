var Token = require('./token')
  , units = require('./units')
  , captures

var Lexer = module.exports = function Lexer(str, options) {
  options = options || {}
  this.stash = []
  this.indentStack = []
  this.lineno = 1

  this.str = str
    .replace(/\s+$/, '\n')
    .replace(/\r\n?/g, '\n')
}

Lexer.prototype = {

  match: function (type) {
    var re = {
        sep: /^;[ \t]*/
      , space: /^([ \t]+)/
      , url: /^[\/:@.;?&=*!,<>#%0-9]/
      , operator: /^([.]{2,3}|[~^$*|]=|[-+*\/%]=?|[,:!~&\[\]])[ \t]*/
      , atrule: /^@(import|(?:-(\w+)-)?keyframes|charset|font-face|page)[ \t]*/
      , important: /^!important[ \t]*/
      , brace: /^([{}])[ \t]*/
      , comment: /^\/\*(?:[^*]|\*+[^\/*])*\*+\/|^\/\/.*/
      , paren: /^([()])[ \t]*/
      , 'function': /^(-*[_a-zA-Z$][-\w\d$]*)\([ \t]*/
      , id: /^(@)?(-*[_a-zA-Z$][-\w\d$]*)/
      , string: /^("[^"]*"|'[^']*')[ \t]*/
      , color: /^#([a-fA-F0-9]{3,8})[ \t]*/
      , unit: new RegExp('^(-)?(\\d+\\.\\d+|\\d+|\\.\\d+)(' + units.join('|') + ')?[ \\t]*')
      , selector: /^[^{\n,]+/
    }

    return re[type].exec(this.str);
  },

  tokenize: function () {
    var tok
      , tmp = this.str
      , buf = []

    while ((tok = this.next()).type !== 'eos') {
      buf.push(tok)
    }

    this.str = tmp
    this.prevIndents = 0

    buf.push(tok);

    return buf
  },

  peek: function () {
    return this.lookahead(1)
  },

  lookahead: function (n) {
    var fetch = n - this.stash.length

    while (fetch-- > 0)
      this.stash.push(this.advance())

    return this.stash[--n]
  },

  skip: function (len) {
    this.str = this.str.substr(Array.isArray(len)
      ? len[0].length
      : len)
  },

  next: function () {
    var tok = this.stashed() || this.advance()
    switch (tok.type) {
      case 'newline':
      case 'indent':
        ++this.lineno
        break
      case 'outdent':
        (this.prev.type !== 'outdent') && ++this.lineno
    }

    this.prev = tok
    tok.lineno = this.lineno
    return tok
  },

  stashed: function () {
    return this.stash.shift()
  },

  advance: function() {
    return this.eos()
      || this.sep()
      || this.url()
      || this.atrule()
      || this.comment()
      || this.newline()
      || this.important()
      || this['function']()
      || this.brace()
      || this.paren()
      || this.color()
      || this.string()
      || this.unit()
      || this.id()
      || this.operator()
      || this.space()
      || this.selector()
  },

  /* Token types */

  eos: function() {
    if (this.str.length) return
    if (this.indentStack.length) {
      this.indentStack.shift()
      return new Token('outdent')
    } else {
      return new Token('eos')
    }
  },

  url: function() {
    if (!this.isURL) return;
    if (captures = this.match('url')) {
      this.skip(captures);
      return new Token('literal', captures[0]);
    }
  },

  atrule: function() {
    if (captures = this.match('atrule')) {
      this.skip(captures);
      var type = captures[1];
      if (captures[2]) type = 'keyframes';
      return new Token('atrule', type);
    }
  },

  id: function() {
    if (captures = this.match('id')) {
      this.skip(captures)
      return new Token('id', captures[2])
    }
  },

  sep: function() {
    if (captures = this.match('sep')) {
      this.skip(captures)
      return new Token(';')
    }
  },

  space: function() {
    if (captures = this.match('space')) {
      this.skip(captures)
      return new Token('space')
    }
  },

  operator: function() {
    if (captures = this.match('operator')) {
      var op = captures[1]
      this.skip(captures)
      this.isURL = false;
      return new Token(op)
    }
  },

  important: function() {
    if (captures = this.match('important')) {
      this.skip(captures)
      return new Token('id', '!important')
    }
  },

  brace: function() {
    if (captures = this.match('brace')) {
      this.skip(1)
      var brace = captures[1]
      return new Token(brace)
    }
  },

  paren: function() {
    if (captures = this.match('paren')) {
      var paren = captures[1]
      this.skip(captures)
      if (')' == paren) this.isURL = false
      return new Token(paren)
    }
  },

  comment: function() {
    if (capture = this.match('comment')) {
      var lines = capture[0].split('\n').length
      this.lineno += lines
      this.skip(captures);
      return new Token('comment', captures[0])
    }
  },

  'function': function () {
    if (captures = this.match('function')) {
      this.skip(captures[1].length)
      var name = captures[1]
      this.isURL = 'url' == name
      return new Token('function', name)
    }
  },

  newline: function() {
    var re

    if (this.indentRe) {
      captures = this.indentRe.exec(this.str)
    } else {
      re = /^\n([\t]*)[ \t]*/ // tabs
      captures = re.exec(this.str)

      if (captures && !captures[1].length) {
        re = /^\n([ \t]*)/ // spaces
        captures = re.exec(this.str)
      }

      if (captures && captures[1].length)
        this.indentRe = re
    }

    if (captures) {
      var tok
        , indents = captures[1].length

      this.skip(captures)

      if (this.str[0] === ' ' || this.str[0] === '\t') {
        throw new Error('Invalid indentation. You can use tabs or spaces to indent, but not both.')
      }

      if (this.str[0] == '\n') {
        ++this.lineno
        return this.advance()
      }

      // Outdent
      if (this.indentStack.length && indents < this.indentStack[0]) {
        while (this.indentStack.length && this.indentStack[0] > indents) {
          this.stash.push(new Token('outdent'))
          this.indentStack.shift()
        }
        tok = this.stash.pop()
      // Indent
      } else if (indents && indents != this.indentStack[0]) {
        this.indentStack.unshift(indents)
        tok = new Token('indent')
      // Newline
      } else {
        tok = new Token('newline')
      }
      return tok
    }
  },

  unit: function() {
    if (captures = this.match('unit')) {
      this.skip(captures)
      var n = parseFloat(captures[2])
      if (captures[1] == '-') n = -n
      var unit = {
          value: n
        , measure: captures[3] || 'px'
      }
      return new Token('unit', unit)
    }
  },

  string: function() {
    if (captures = this.match('string')) {
      var str = captures[1]
        , quote = captures[0][0]
      this.skip(captures)
      str = str.slice(1, -1).replace(/\\n/g, '\n')
      return new Token('string', str)
    }
  },

  color: function() {
    if (captures = this.match('color')) {
      this.skip(captures)
      var rgb = captures[1]
        , len = rgb.length
        , single = len >= 3 && len <= 4
        , r = parseInt(single ? rgb[0] + rgb[0] : rgb.substr(0, 2), 16)
        , g = parseInt(single ? rgb[1] + rgb[1] : rgb.substr(2, 2), 16)
        , b = parseInt(single ? rgb[2] + rgb[2] : rgb.substr(4, 2), 16)
        , a = len % 4
                ? 1
                : parseInt(single ? rgb[3] + rgb[3] : rgb.substr(6, 2), 16) / 255
        , color = {
            r: r
          , g: g
          , b: b
          , a: a
          , raw: captures[0]
        }
      return new Token('color', color)
    }
  },

  selector: function() {
    if (captures = this.match('selector')) {
      var selector = captures[0]
      this.skip(captures)
      return new Token('selector', selector)
    }
  }
}
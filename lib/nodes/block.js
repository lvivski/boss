    ...
    if (env.compress) {
      ...
      for (var i = 0, prop, len = this.nodes.length; i < len; ++i) {
        ...
        if (
          ...
          && (prop = node.property.split('-'))
          && ~shorthands.compress.indexOf(prop[0])
        ) {
          !nodes[prop[0]] && (nodes[prop[0]] = [])
          nodes[prop[0]].push({
            index: i
          , side: prop[1]
          , value: node.value
      ...
      for (prop in nodes) {
        if(compressed = compressProperties(nodes[prop])) {
          ...
          node.property = prop
          node.value = compressed.value.eval(env)
...
function compressProperties(arr) {
  ...
  arr.forEach(function (node, i) {
    ...
    if (!node.side) {
      ...
      expr.nodes[shorthands.order.indexOf(node.side)] = node.value.nodes[0]
    }
  })
